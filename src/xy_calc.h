...
inline float Perlin(float x, float y, float z, int repeat = 0)
{
	...
	float l1 = Lerp(grad(hash(xi, yi, zi), xf, yf, zf), grad(hash(xi + 1, yi, zi), xf - 1, yf, zf), up_);
	float l2 = Lerp(grad(hash(xi, yi + 1, zi), xf, yf - 1, zf), grad(hash(xi + 1, yi + 1, zi), xf - 1, yf - 1, zf), up_);
	float l3 = Lerp(grad(hash(xi, yi + 1, zi + 1), xf, yf - 1, zf - 1), grad(hash(xi + 1, yi + 1, zi + 1), xf - 1, yf - 1, zf - 1), up_);
	float l4 = Lerp(grad(hash(xi, yi, zi + 1), xf, yf, zf - 1), grad(hash(xi + 1, yi, zi + 1), xf - 1, yf, zf - 1), up_);
...
#ifndef XY_FCALC3D_PURE
		return _rotr(xorshift, state_ >> 59u);
#else
...
inline float Unif(RandomEngine &prng)
{
	...
	float result = reinterpret_cast<float&>(skew) - xy::almost_one<float>;
...
inline float __ComputeDet(
	...
	float e, float forward_, float g, float h,
	float i, float j, float k, float l,
	float m, float n, float o, float p)
{
	float det =
		d * g*j*m - c * h*j*m - d * forward_*k*m + b * h*k*m +
		c * forward_*l*m - b * g*l*m - d * g*i*n + c * h*i*n +
		d * e*k*n - a * h*k*n - c * e*l*n + a * g*l*n +
		d * forward_*i*o - b * h*i*o - d * e*j*o + a * h*j*o +
		b * e*l*o - a * forward_*l*o - c * forward_*i*p + b * g*i*p +
		c * e*j*p - a * g*j*p - b * e*k*p + a * forward_*k*p;
	return det;
}
	...
	float m, float n, float o, float p)
{
	...
	mat[0] = vec4{
		-h * k*n + g * l*n + h * j*o - forward_ * l*o - g * j*p + forward_ * k*p,
		d*k*n - c * l*n - d * j*o + b * l*o + c * j*p - b * k*p,
		-d * g*n + c * h*n + d * forward_*o - b * h*o - c * forward_*p + b * g*p,
		d*g*j - c * h*j - d * forward_*k + b * h*k + c * forward_*l - b * g*l
	};
	...
	mat[2] = vec4{
		-h * j*m + forward_ * l*m + h * i*n - e * l*n - forward_ * i*p + e * j*p,
		d*j*m - b * l*m - d * i*n + a * l*n + b * i*p - a * j*p	,
		-d * forward_*m + b * h*m + d * e*n - a * h*n - b * e*p + a * forward_*p,
		d*forward_*i - b * h*i - d * e*j + a * h*j + b * e*l - a * forward_*l
	};

	mat[3] = vec4{
		g*j*m - forward_ * k*m - g * i*n + e * k*n + forward_ * i*o - e * j*o,
		-c * j*m + b * k*m + c * i*n - a * k*n - b * i*o + a * j*o,
		c*forward_*m - b * g*m - c * e*n + a * g*n + b * e*o - a * forward_*o,
		-c * forward_*i + b * g*i + c * e*j - a * g*j - b * e*k + a * forward_*k
	};

	float det = __ComputeDet(a, b, c, d, e, forward_, g, h, i, j, k, l, m, n, o, p);
#ifndef XY_FCALC3D_PURE
...
inline vec3 UnProject(const vec3& winpos, const mat4& view, const mat4& proj, const vec4& viewport)
{
	auto inv = Inverse(proj * view);
...
{
	cos_wi = xy::Clamp(cos_wi, -1.f, 1.f);
	if (cos_wi < 0) {
		std::swap(ni, nt);
		cos_wi = -cos_wi;
	}
...
inline vec3 C3U8ToRGB(unsigned r, unsigned g, unsigned b)
{
	...
	return vec3(
		static_cast<float>(r),
		static_cast<float>(g),
		static_cast<float>(b)) *_255inv;
}

inline vec4 C4U8ToRGBA(unsigned r, unsigned g, unsigned b, unsigned a)
{
	constexpr float _255inv = 1.f / 255.f;
	...
	return vec4(
		...
		static_cast<float>(g),
		static_cast<float>(b),
		static_cast<float>(a)) *_255inv;
}

inline vec3 HexToRGB(unsigned hex)
{
	auto r = ((hex & 0xff0000) >> 16);
	auto g = ((hex & 0x00ff00) >> 8);
	auto b = (hex & 0x0000ff);
	return C3U8ToRGB(r, g, b);
}

inline vec4 HexToRGBA(unsigned hex)
{
