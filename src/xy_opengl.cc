...
std::string ResolveShaderInclude(const std::unordered_map<std::string, std::string> &directive_replace, std::string shader_content)
{
	char include_directive = '@';

	std::string new_shader_content;
	new_shader_content.reserve(shader_content.size());

	int kth_char = 0;
	while (kth_char < shader_content.size()) {
		if (shader_content[kth_char] != include_directive) {
			new_shader_content.push_back(shader_content[kth_char]);
			++kth_char;
		}
		else {
			int count = 0;
			for (; count < 128; ++count)
				if (shader_content[kth_char + count + 1] == include_directive)
					break;
			if (count == 128)
				XY_Die("include_directive too long.");

			if (count > 0) {
				auto name = shader_content.substr(kth_char + 1, count);
				if (directive_replace.count(name) == 0)
					xy::Print("ResolveShaderInclude: {} not found.", name);
				else
					new_shader_content += directive_replace.at(name);
			}
			kth_char += (count + 2);
		}
	}

	return new_shader_content;
}
...
HairGAsset MakeHairGAsset(const HairAsset & asset)
{
	...
	std::vector<unsigned> vcounts;
	{
		unsigned kth_vert = 0;
		for (auto vcount : asset.vcounts) {
			for (int i = 0; i < vcount; ++i)
				vcounts.push_back(kth_vert++);
			vcounts.push_back(0xffff);
		}
		gasset.num_indices = vcounts.size();
	}

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, gasset.bufs[2]);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned) * vcounts.size(), vcounts.data(), GL_STATIC_DRAW);
...
void HairGAsset::DrawElements(const std::vector<int>&& attribs) const
{
	glBindVertexArray(vao);
	for (auto attrib : attribs)
		glEnableVertexAttribArray(attrib);
	...
	glDrawElements(GL_LINE_STRIP, num_indices/5, GL_UNSIGNED_INT, (GLvoid*)0);
...
void ArcballCamera::HandleInput(const XYInput & input)
{
	...
	if (tracking && !tracking_now) {
		...
		up_ = track_rot_ * up_;
		if (std::abs(xy::Dot(up_, forward_)) > 0.000001) {
	...
	if (tracking&&tracking_now) {
		...
		if ((nhit - first_hit_).Norm() < 0.000001)
			return;
